...
class Keepitcollections extends StatefulWidget {
  static const String routeName = '/keepitcollection';
  const Keepitcollections({super.key});
  // final int value;
  // final int tvalue;
  //Keepitcollections(this.value, this.tvalue, {Key? key}) : super(key: key);
  //Keepitcollections({Key? key}) : super(key: key);

  void getTags() {
    _KeepitcollectionsState().getTags();
  }

  @override
  State<Keepitcollections> createState() => _KeepitcollectionsState();
}
...
class _KeepitcollectionsState extends State<Keepitcollections>
  ...
  static String? selectedItem = 'Name A to Z';
  ...
  void initState() {
    ...
    // print("Last argument is $arguments");
    // if (arguments == '' || arguments == null) {
    //   value = 0;
    // } else {
    //   value = arguments['tabIndex'];
    // }
    _tabController = TabController(vsync: this, length: this.tvalue);
  }
  ...
  void doSingleSelection(String path, String name) {
    setState(() {
      //print(path);
      if (selectItems.contains(path)) {
        ...
        showModalBottomSheet(
          ...
          tabtimer =
              Timer.periodic(Duration(seconds: 1), (Timer tabtimer) async {
            SharedPreferences prefs = await SharedPreferences.getInstance();
            bool isRedirect = prefs.getBool('isRedirect') ?? false;
            if (isRedirect) {
              tabtimer.cancel();
              setState(() {
                changeTabIns = isRedirect;
                prefs.setBool('isRedirect', false);
              });
            }
          });
          setState(() {
  ...
  void get_collections() async {
    var foo = await controls.Controls().get_all_collections('collections');
    ...
    return WillPopScope(
        ...
        child: Consumer(
          ...
          builder:
            ...
            List list = provider.keepfiles;
            //provider.keepFiles(list);
            if (changeTabs_name_ins == true) {
              if (changeTab_name != "All") {
                int index = provider.keepfileTabs
                ...
                if (index == 0 || index > 0) {
                  ...
                  Future.delayed(Duration(seconds: 1), () {
                    ...
                    print("P tab view loaded ${changeTab_name}");
                  });
                } else {
                  index = 0;
                }
                changeTab = index;
                changeTabs_name_ins = false;
            ...
            return DefaultTabController(
              length: provider.keepfileTabs.length,
              ...
              child: Scaffold(
                appBar: AppBar(
                  leading: isMultiSelectionEnabled
                      ...
                      : GestureDetector(
                          onTap: () {
                            ...
                            selectItems.clear();
                            done_action = false;
                            provider.switchCurrentFiles(
                                provider.keepfiles, 'All', 'keep');
                            Navigator.pushAndRemoveUntil(
                                context,
                                MaterialPageRoute(builder: (context) => HomeScreen()),
                                (Route<dynamic> route) => false);
                  ...
                  bottom: PreferredSize(
                    ...
                    child: TabBar(
                        ...
                        isScrollable:
                            provider.keepfileTabs.length < 3 ? false : true,
                        tabs: Constants.map<Widget>(
                          provider.keepfileTabs,
                          (index, label) {
                        ...
                        onTap: (val) {
                          ...
                          changeTab_temp = val;
                          changeTab_name = provider.keepfileTabs[val];

                          setState(() {
                            totalTabs = provider.keepfileTabs.length;
                            provider.switchCurrentFiles(provider.keepfiles,
                                provider.keepfileTabs[val], 'keep');
                          });
                        }),
                  ),
                ...
                body: Container(
                  ...
                  child: filters.isEmpty
                      ? Visibility(
                          visible: provider.onePackKeep.isNotEmpty,
                          replacement: Center(
                          ...
                          child: TabBarView(
                              ...
                              children: [
                                for (var tab in provider.keepfileTabs)
                                  buildTabView(tab),
                              ]),
                        )
                      : Container(
                  ...
                  child: FloatingActionButton.extended(
                    ...
                    onPressed: () {
                      ...
                      showModalBottomSheet(
                        ...
                        tabtimer = Timer.periodic(Duration(seconds: 1),
                            (Timer tabtimer) async {
                          SharedPreferences prefs =
                              await SharedPreferences.getInstance();
                          bool isRedirect =
                              prefs.getBool('isRedirect') ?? false;
                          if (isRedirect) {
                            tabtimer.cancel();
                            setState(() {
                              changeTabIns = isRedirect;
  ...
  void create_object_of_filtered_list() async {
    ...
    for (FileSystemEntity file in temporary_paths) {
      ...
      String path = file.path;
      var kfolder = '${file.path.split('/')[file.path.split('/').length - 3]}';
  ...
  Widget FilteredListWidget(List list_obj) => isGrid
      ? GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            var thumbnail =
                sortedItems[index].thumbnail.toString().replaceAll("'", "");
  ...
  buildTabView(name) {
    Future.delayed(Duration(milliseconds: 1000), () async {
      ...
      // setState(() {
      if (_istabResumed) {
        print("Tab has resumed on the server");
        changeTabs_name_ins = false;
        changeTab_name = 'All';
        prefs.setBool('istabResumed', false);
      }
    ...
    return Consumer(builder:
      ...
      timer = Timer.periodic(Duration(seconds: 1), (Timer t) {
        if (changeTabIns == true) {
          if (provider.keepfileTabs.length < 2) {
            print("Tabs is not loaded fully: ${provider.keepfileTabs.length}");
          } else {
            setState(() {
              changeTabIns = false;
              changeTabs_name_ins = true;
      ...
      if (name == "All") {
        List list = provider.keepfiles;
        List list_obj = provider.onePackKeep;

        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            //print("switch val is: ${switch_value}");
            provider.switchCurrentFiles(provider.keepfiles, name, 'keep');
            List list_obj = provider.onePackKeep;
            switch_value = false;
          }
        });
        ...
        List list = provider.currentKeep;
        List list_obj = provider.onePackKeep;
          ...
          itemBuilder: (context, index) {
            ...
            return Stack(alignment: Alignment.center, children: [
              ...
              Positioned(
                  top: 10,
                  right: 10,
                  // child: Text(file_name.toString().substring(0, 20), style: TextStyle(color: Colors.white))
                  child: Center(
                    child: Text(file_size.toString(),
                        style: TextStyle(color: Colors.white),
                        textAlign: TextAlign.right),
      ...
      : ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            var file_date =
                DateFormat('dd/MM/yyyy').format(sortedItems[index].file_date);
            String keep_status = sortedItems[index].keep_status.toString();
            print("Last file path is: ${thumbnail}");
