...
class _ImagesState extends State<Images> {
  ...
  int current_tab = 0;
  static List<String> image_files = [];
  static int _tab_length = 1;
  static List<String> _tab_paths = [];
  static List<FileSystemEntity> all_images = [];
    ...
    setState(() {
      ...
      if (selectItems.contains(path)) {
        ...
        showModalBottomSheet(
          ...
          // tabtimer = Timer.periodic(Duration(seconds: 1), (Timer tabtimer) async {
                
          //   bool isRedirect = ShareP.preferences.getBool('isRedirect') ?? false;
          //   if (isRedirect) {
          //     setState(() {
          //       tabtimer.cancel();
          //       changeTabIns = isRedirect;
          //       ShareP.preferences.setBool('isRedirect', false);
          //     });
          //   }
          // });
          setState(() async{
            ...
            if (isRedirect) {
                changeTabIns = isRedirect;
                await ShareP.preferences.setBool('isRedirect', false);
            }
    ...
    return WillPopScope(
        ...
        child: Consumer(
          ...
          builder:
            ...
            List list = provider.images;
            //provider.keepFiles(list);
            if (changeTabs_name_ins == true) {
              if (changeTab_name != "All") {
                int index = provider.imageTabs.indexWhere((element) => element == changeTab_name);
              ...
              child: Scaffold(
                  ...
                  bottom: PreferredSize(
                    ...
                    child: TabBar(
                        ...
                        onTap: (val) {
                          ...
                          changeTab_temp = val;
                          changeTab_name = provider.imageTabs[val];

                          setState(() {
                            
                            current_tab = val;
                            totalTabs = provider.imageTabs.length;
                            provider.switchCurrentFiles(provider.images,
                                provider.imageTabs[val], 'images');
                            selectItems.clear();
                            isMultiSelectionEnabled = false;
                            print("Images are : ${provider.images}");
                          });
                        }),
                  ),
                      ...
                      onSelected: (value) async {
                        ...
                        switch (value) {
                          case _MenuValues.SelectAll:
                            // List list = provider.currentImageFiles;
                            // }else{
                            ...
                            // }
                              ...
                              if (current_tab == 0) {
                                ...
                                list = provider.currentImageFiles;
                              }
                ...
                body: Container(
                    ...
                    child: filters.isEmpty
                        ? Visibility(
                            visible: provider.onePackImages.isNotEmpty,
                            replacement: Center(
                            ...
                            child: TabBarView(
                                ...
                                children: [
                                  for (var tab in provider.imageTabs)
                                    buildTabView(tab),
                                ]),
                          )
                        : Container(
                ...
                child: InkWell(
                  onTap: (() {
                    setState(() {
                      ...
                      print('The scrol position is ${scrollOffset}');
                    });
    ...
    return Consumer(builder:
      ...
      timer = Timer.periodic(Duration(seconds: 1), (Timer t) {
        if (changeTabIns == true) {
          if (provider.imageTabs.length < 2) {
            print("Tabs is not loaded fully: ${provider.imageTabs.length}");
          } else {
            setState(() {
              changeTabIns = false;
      ...
      if (name == "All") {
        List list = provider.images;
        List list_obj = provider.onePackImages;
        List<FileOBJ> search = provider.onePackImages;
        list_Search = search;

        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            //print("switch val is: ${switch_value}");
            provider.switchCurrentFiles(provider.images, name, 'images');
            List list_obj = provider.onePackImages;
            switch_value = false;
            isMultiSelectionEnabled = false;
          }
            ...
            return Stack(alignment: Alignment.center, children: [
              GridTile(
                child: InkWell(
                  onTap: (() {
                    scrollOffset = scrollController.position.pixels;
                    print('Scroll podition is ${scrollOffset.toString()}');
