...
class _AudioState extends State<Audio> {
  ...
  static List<String> audio_files = [];
  static int _tab_length = 1;
  static List<String> _tab_paths = [];
  static List<FileSystemEntity> all_audio = [];
            ...
            actions: [
              ...
              TextButton(
                onPressed: () async {
                  ...
                  print("Notice is " + notice.toString());
              ...
              child: Scaffold(
                  ...
                  bottom: PreferredSize(
                    ...
                    child: TabBar(
                        ...
                        onTap: (val) {
                          ...
                          changeTab_temp = val;
                          changeTab_name = provider.audioTabs[val];

                          setState(() {
                            totalTabs = provider.audioTabs.length;
                            current_tab = val;
                            provider.switchCurrentFiles(provider.audio,
                                provider.audioTabs[val], 'audio');
                            //print("audio are : ${provider.audio}");
                            selectItems.clear();
                            isMultiSelectionEnabled = false;
                          });
                        }),
                          ...
                          case _MenuValues.SelectAll:
                            ...
                            if (filters.length == 0) {
                              ...
                              if (current_tab == 0) {
                                ...
                                list = provider.currentAudioFiles;
                              }
                ...
                body: Container(
                    ...
                    child: filters.isEmpty
                        ? Visibility(
                            visible: provider.onePackAudio.isNotEmpty,
                            replacement: Center(
                            ...
                            child: TabBarView(
                                ...
                                children: [
                                  for (var tab in provider.audioTabs)
                                    buildTabView(tab),
                                ]),
                          )
                        : Container(
    ...
    return Consumer(builder:
      ...
      timer = Timer.periodic(Duration(seconds: 1), (Timer t) {
        if (changeTabIns == true) {
          if (provider.audioTabs.length < 2) {
            print("Tabs is not loaded fully: ${provider.audioTabs.length}");
          } else {
            setState(() {
              changeTabIns = false;
      ...
      if (name == "All") {
        List list = provider.audio;
        List list_obj = provider.onePackAudio;
        List<FileOBJ> search = provider.onePackAudio;
        list_Search = search;

        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            //print("switch val is: ${switch_value}");
            provider.switchCurrentFiles(provider.audio, name, 'audio');
            List list_obj = provider.onePackAudio;
            switch_value = false;
            selectItems.clear();
            isMultiSelectionEnabled = false;
            //provider.reloaded == false;
          }
        });
        ...
        List list = provider.currentAudioFiles;
        List list_obj = provider.onePackAudio;
        List<FileOBJ> search = provider.onePackAudio;
        list_Search = search;
