...
class _VideosState extends State<Videos> {
  ...
  static List<String> video_files = [];
  static int _tab_length = 1;
  static List<String> _tab_paths = [];
  static List<FileSystemEntity> all_video = [];
    ...
    return WillPopScope(
        ...
        child: Consumer(
          ...
          builder:
            ...
            List list = provider.video;
            if (changeTabs_name_ins == true) {
              if (changeTab_name != "All") {
                int index = provider.videoTabs
            ...
            return DefaultTabController(
              length: provider.videoTabs.length,
              ...
              child: Scaffold(
                appBar: AppBar(
                  ...
                  bottom: PreferredSize(
                    ...
                    child: TabBar(
                        ...
                        isScrollable:
                            provider.videoTabs.length < 3 ? false : true,
                        ...
                        onTap: (val) {
                          ...
                          changeTab_temp = val;
                          changeTab_name = provider.videoTabs[val];

                          setState(() {
                            totalTabs = provider.videoTabs.length;
                            current_tab = val;
                            provider.switchCurrentFiles(provider.video,
                                provider.videoTabs[val], 'videos');
                            print("video are : ${provider.video}");
                            selectItems.clear();
                            isMultiSelectionEnabled = false;
                          });
                        }),
                      ...
                      onSelected: (value) async {
                        ...
                        switch (value) {
                          case _MenuValues.SelectAll:
                            // List list = provider.currentVideosFiles;
                            ...
                            if (filters.length == 0) {
                              ...
                              if (current_tab == 0) {
                                ...
                                list = provider.currentVideosFiles;
                              }
                ...
                body: Container(
                    ...
                    child: filters.isEmpty
                        ? Visibility(
                            visible: provider.onePackVideos.isNotEmpty,
                            replacement: Center(
                            ...
                            child: TabBarView(
                                ...
                                children: [
                                  for (var tab in provider.videoTabs)
                                    buildTabView(tab),
                                ]),
                          )
                        : Container(
    ...
    return Consumer(builder: (BuildContext context, CategoryProvider provider, Widget? child) {
      timer = Timer.periodic(Duration(seconds: 1), (Timer t) {
        if (changeTabIns == true) {
          if (provider.videoTabs.length < 2) {
            print("Tabs is not loaded fully: ${provider.videoTabs.length}");
          } else {
            setState(() {
              changeTabIns = false;
      ...
      if (name == "All") {
        List list = provider.video;
        List list_obj = provider.onePackVideos;
        List<FileOBJ> search = provider.onePackVideos;
        list_Search = search;
        print("Video Files are : ${list}");
        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            //print("Video Files are : ${list}");
            provider.switchCurrentFiles(provider.video, name, 'videos');
            List list_obj = provider.onePackVideos;
            switch_value = false;
            isMultiSelectionEnabled = false;
          }
      ...
      ? GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            var file_extension = file_name
                .toString()
                .substring(file_name.toString().lastIndexOf("."))
                .substring(1);
            String keep_status = sortedItems[index].keep_status.toString();
