...
class All extends StatefulWidget {
  static const String routeName = '/all_files';
  const All({super.key});
...
class _AllState extends State<All> {
  ...
  int current_tab = 0;
  static List<String> all_files = [];
  static int _tab_length = 1;
  static List<String> _tab_paths = [];
  static List<FileSystemEntity> all_all = [];
          ...
          builder:
            ...
            if (changeTabs_name_ins == true) {
              if (changeTab_name != "All") {
                int index = provider.allfileTabs.indexWhere((element) => element == changeTab_name);
                if (index == 0 || index > 0) {
                ...
                Future.delayed(Duration(seconds: 1), () {
                  ...
                  print('Rturned scroll position ${scrollOffset.toString()}');
                });
              }else{
                  scrollController.animateTo(scrollOffset,
                      duration: const Duration(milliseconds: 100),
            ...
            return DefaultTabController(
              length: provider.allfileTabs.length,
              ...
              child: Scaffold(
                appBar: AppBar(
                  ...
                  title: Text(
                      isMultiSelectionEnabled ? getSelectedItemCount() : "All"),
                  ...
                  bottom: PreferredSize(
                    ...
                    child: TabBar(
                        ...
                        isScrollable:
                            provider.allfileTabs.length < 3 ? false : true,
                        tabs: Constants.map<Widget>(
                          provider.allfileTabs,
                          (index, label) {
                        ...
                        onTap: (val) {
                          ...
                          changeTab_temp = val;
                          changeTab_name = provider.allfileTabs[val];

                          setState(() {
                            current_tab = val;
                            totalTabs = provider.allfileTabs.length;
                            provider.switchCurrentFiles(provider.allfiles,
                                provider.allfileTabs[val], 'all');
                            print("all are : ${provider.allfiles}");
                            selectItems.clear();
                            isMultiSelectionEnabled = false;
                          });
                        }),
                      ...
                      onSelected: (value) async {
                        ...
                        switch (value) {
                          case _MenuValues.SelectAll:
                            // List list = provider.currentAllFiles;
                            ...
                            // }
                              ...
                              List list;
                              print('The current tab ${current_tab}');
                              if (current_tab == 0) {
                                list = provider.allfiles;
                              } else {
                                list = provider.currentAllFiles;
                              }
                ...
                body: Container(
                    ...
                    child: filters.isEmpty
                        ? Visibility(
                            visible: provider.onePackAll.isNotEmpty,
                            replacement: Center(
                            ...
                            child: TabBarView(
                                ...
                                children: [
                                  for (var tab in provider.allfileTabs)
                                    buildTabView(tab),
                                ]),
                          )
                        : Container(
  ...
  buildTabView(name) {
    ...
    //print('All Build: Switch Value is ${switch_value}');
    print("All tab MultiSelection: ${isMultiSelectionEnabled}");
    // istabResumed =
    //     Timer.periodic(Duration(seconds: 1), (Timer tabresumed) async {
    //   SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    //print("All pages: server name ${name}");
    return Consumer(builder:
        (BuildContext context, CategoryProvider provider, Widget? child) {
      timer_all = Timer.periodic(Duration(seconds: 1), (Timer t) {
        //print("All pages Server all files timer is running fine");
        if (changeTabIns == true) {
          if (provider.allfileTabs.length < 2) {
            print(
                "All pages Tabs is not loaded fully: ${provider.allfileTabs.length}");
          } else {
            setState(() {
              changeTabIns = false;
      ...
      if (name == "All") {
        List list = provider.allfiles;
        List list_obj = provider.onePackAll;
        List<FileOBJ> search = provider.onePackAll;
        list_Search = search;

        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            //print("switch val is: ${switch_value}");
            provider.switchCurrentFiles(provider.allfiles, name, 'all');
            List list_obj = provider.onePackAll;
            switch_value = false;
            isMultiSelectionEnabled = false;
          }
        ...
        Future.delayed(Duration(milliseconds: 1000), () {
          if (switch_value == true) {
            print("All pages: Switch val is: ${switch_value}");
            provider.switchCurrentFiles(provider.allfiles, name, 'all');
            List list_obj = provider.onePackAll;
            switch_value = false;
            isMultiSelectionEnabled = false;
    ...
    if (selectItems.contains(path)) {
      ...
      checkSub().then((value) {
        showModalBottomSheet(
          ...
          selectItems.clear();
          _AllState obj = _AllState();
          switch_value = true;
          obj.buildTabView("All");
          searchResults = list_Search;
        });
      });
    }
  }
